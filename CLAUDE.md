...
This is the operating manual for Claude Code agents working in this repository. It is loaded automatically before every task. It takes precedence over inferred behavior and general training.

**Multi-tool note:** This framework supports Claude Code, OpenAI Codex, Windsurf, and GitHub Copilot. Canonical command and agent specs live in `.writing-framework/commands/` and `.writing-framework/agents/`. This file and `.claude/` are the Claude Code adapter. Other tools use `.codex/`, `.windsurf/`, and `.copilot/`. When this file conflicts with a general pattern, follow this file.
...
1. **Read CLAUDE.md (this file).** It is always loaded first. If CLAUDE.md has been updated since your last read, re-read it before proceeding.
...
| Final gate | Does the document clear the formal QA gate? | Aggregates the perspective reports into the authoritative gate decision and next action. |

**QA verdicts:**
- Perspective review reports use `ACCEPT`, `REVISE`, or `BLOCK`
- Formal quality gates use `PASS`, `FAIL`, or `OVERRIDE`
- `REVISE` and `FAIL` require explicit remediation guidance before advancement
...
- Use `/import-framework` for pulling framework updates into the current repository.
- Use `/export-framework` for publishing the framework to another repository or to a portable bundle.
...
| `.claude/commands/` | **Claude Code adapter** — slash command files that wrap `.writing-framework/commands/` specs in Claude Code format. | Claude Code | Phase 1: scaffold |
| `.claude/hooks/` | Claude Code hook definitions adapted from `.writing-framework/hooks/`. | `lead-orchestrator` | Phase 1: scaffold |
...
| `sync/export-packs/` | Portable framework bundles ready for distribution. | `/export-framework` | Phase 6: implemented |
| `sync/import-packs/` | Legacy staging area for imported bundles and compatibility flows. | `/import-framework` | Phase 6: implemented |
...
| `review_report` | `.writing-framework/schemas/review_report.json` | QA review: review ID, draft reference, applied perspectives, issue list, `gate_decision` (`ACCEPT`/`REVISE`/`BLOCK`), counts, and provenance fields | Phase 4 |
...
| `quality_gate` | `.writing-framework/schemas/quality_gate.json` | Gate evaluation: gate ID, phase, artifact reference, `decision` (`PASS`/`FAIL`/`OVERRIDE`), per-criterion results, unmet criteria, next action, and provenance fields | Phase 4 |
...
The repository now has implemented command and adapter surfaces through Phase 12, with end-to-end verification still pending. The main implemented path is the Phase 11 writing flow plus the Phase 12 QA flow; many canonical agent specs remain scaffold-level.

### Command Implementation Status

Core specs live in `.writing-framework/commands/`. Claude Code adapters live in `.claude/commands/`. Other tool adapters are in `.codex/`, `.windsurf/`, `.copilot/`.

| Command Group | Commands | Core Spec | Claude Adapter | Implemented In |
|---------------|----------|-----------|----------------|----------------|
| Foundation | `/help`, `/session-start`, `/project-scan`, `/status`, `/whats-next`, `/explain-workflow` | Mixed | Mixed | Phase 2 |
| Discovery | `/discovery`, `/discovery-agent`, `/discovery-simulate-user`, `/requirements-brief` | Mixed | Mixed | Phase 2 |
| Research | `/research`, `/validate-research`, `/synthesize-research`, `/source-gap-check`, `/evidence-map` | Mixed | Mixed | Phase 3 |
| Editorial | `/write-brief`, `/validate-brief`, `/write-outline`, `/validate-outline`, `/draft-section`, `/draft-document`, `/merge-draft`, `/rewrite`, `/line-edit`, `/compress`, `/voice-pass`, `/canon-check`, `/publication-check` | Phase 11 core path active; remaining surfaces mixed | Phase 11 core adapters active; remaining surfaces mixed | Phase 3 |
| QA | `/qa-reader`, `/qa-skeptic`, `/qa-domain`, `/qa-style`, `/qa-coherence`, `/qa-ai-stink`, `/qa-final` | Active (Phase 12) | Active (Phase 12) | Phase 4 |
...
| Guides | `/add-guide`, `/update-guide`, `/find-guides`, `/guide-gap-check`, `/guide-link`, `/guide-promote`, `/guide-deprecate` | Mixed | Mixed | Phase 2 |
| Sync | `/import-framework`, `/export-framework`, `/import-principles`, `/export-principles`, `/sync-principles`, `/sync-framework`, `/upgrade-framework`, `/export-pack`, `/import-pack`, `/install-framework`, `/apply-style-pack`, `/apply-doctrine` | Mixed | Mixed | Phase 6 |
| Artifacts | `/write-markdown`, `/write-docx`, `/write-pdf`, `/write-latex`, `/edit-docx`, `/edit-latex`, `/export-docx`, `/export-pdf`, `/normalize-artifact`, `/artifact-validate` | Mixed | Mixed | Phase 5 |
